...
- [../../agents/docs/AIGovernance.md](../../agents/docs/AIGovernance.md) — conhecimento canônico do mecanismo de governança de escrita (`ActionProposal`, Policy Engine, Approval, Recovery Package, rollback, auditoria); seção "Onda 2 — `governed-execute`" registra os aprendizados reutilizáveis da execução real e persistida em produção (SOMA), incluindo a regra de que ajuste de grade PED usa quantidade final absoluta por posição, não delta.
