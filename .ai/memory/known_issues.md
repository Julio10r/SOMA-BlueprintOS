...
- **PDF não é uma conversão literal do HTML.** QuestPDF (biblioteca .NET pura, sem Chromium) reconstrói a estrutura do documento a partir de um parser de blocos Markdown próprio (`MarkdownBlockParser`), garantindo consistência de conteúdo entre os três formatos, mas não fidelidade pixel-a-pixel de layout entre HTML e PDF.
