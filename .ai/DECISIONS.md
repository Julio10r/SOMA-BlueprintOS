...
- *(R1.2)* O papel "Administrador Sênior" citado pelo Bootstrap Mode ainda não está no catálogo de Perfis aprovado (dúvida de produto nº 5/nº 6 de `ComprasFuncional.md`); a O1.2 precisa aprovar esse perfil como parte do catálogo antes de implementar o Bootstrap. **RESOLVIDO pela ADR-0022 (Gate Final da Onda 1, continuação em 12/08/2026):** catálogo inicial de Perfis de negócio aprovado e implementado (entregável #9).
...
## ADR-0022: Escopo Administrativo (Produto × Negócio) e Catálogo Inicial de Perfis de Negócio

**Status:** Aceito

**Contexto:** O Gate Final da Onda 1 (12/08/2026, ver `PROJECT_STATE.md`) identificou uma ambiguidade arquitetural real ao reauditar os 17 controllers administrativos: 5 deles (Alçadas, Regras de Workflow, Regras Orçamentárias, Identity Providers, Configuração de Notificações) recebem `unidadeNegocioId` explicitamente no path, enquanto os demais 12 confiam exclusivamente na Unidade de Negócio da sessão — sem que nenhuma policy ou handler central jamais verificasse **se o ator estava autorizado a administrar aquela Unidade de Negócio especificamente**. A permissão RBAC (`Sistema.Gerenciar`, `Alcada.Gerenciar`, etc.) só respondia "o ator pode executar esta operação?", nunca "em qual BU?" — deixando, na prática, aberta a possibilidade teórica de um titular de uma dessas permissões atravessar Unidades de Negócio pelos 5 controllers de path explícito. O catálogo de Perfis de negócio (dúvida de produto nº 5/nº 6 de `ComprasFuncional.md`, ADR-0020 R1.2, risco aceito) também permanecia pendente — o entregável #9 dependia desta decisão para ser resolvido.

**Decisão:** O Product Owner registra formalmente:

- **D1 — Dois níveis administrativos distintos:** existe **Administrador Sênior** (escopo `Produto`) e **Administrador de BU** (escopo `Negocio`), como dimensão ortogonal ao RBAC. RBAC (Perfil → Permissões) responde **o quê** um ator pode fazer; **escopo administrativo** responde **em qual Unidade de Negócio**. `AUTORIZAÇÃO EFETIVA = PERMISSÃO + ESCOPO`.
- **D2 — Administrador Sênior é cross-BU, mas nunca por confiança implícita:** pode administrar qualquer Unidade de Negócio, mas somente porque o backend verifica explicitamente seu escopo (`EscopoAdministrativo.Produto`) a cada requisição — nunca por simplesmente possuir uma permissão ampla como `Sistema.Gerenciar`, e nunca por confiar em um `unidadeNegocioId` recebido do cliente sem essa verificação.
- **D3 — Administrador de BU (e demais Perfis de negócio) são estritamente mono-BU:** restritos à própria Unidade de Negócio mesmo informando o Id válido de outra — a tentativa é negada (`403`), nunca silenciosamente redirecionada à própria BU nem aceita.
- **D4 — Abstração central obrigatória:** o escopo administrativo é resolvido por uma única abstração reutilizável (`EscopoAdministrativo` no domínio; `EscopoAdministrativoUnidadeNegocio.Autoriza`/`TryResolverUnidadeNegocio` e `EscopoUnidadeNegocioPathFilter` em `BlueprintOS.Api.Authorization`) — nenhum controller compara `Perfil.Nome == "Administrador Sênior"` ou reimplementa a regra isoladamente.
- **D5 — Catálogo inicial de Perfis de negócio (entregável #9):** aprovado o catálogo — **Administrador Sênior** (já existente, ciclo de vida exclusivo do Bootstrap), **Administrador de BU**, **Comprador**, **Aprovador**, **Requisitante**. Não é catálogo eterno/imutável — novos Perfis continuam sendo criados livremente pela Gestão de Perfis (RBAC já existente, ADR-0020 itens 8/10).
- **D6 — Permissões dos Perfis de negócio derivam exclusivamente do catálogo real:** nenhuma permissão fictícia foi criada. Administrador de BU recebe somente as 8 permissões classificadas NEGÓCIO (`Usuario.Gerenciar`, `Perfil.Gerenciar`, `Filial.Gerenciar`, `CentroCusto.Gerenciar`, `UnidadeAlocacao.Gerenciar`, `Workflow.Gerenciar`, `Alcada.Gerenciar`, `Orcamento.Gerenciar`) — nunca `UnidadeNegocio.Gerenciar`, `ConfiguracaoErp.Gerenciar` ou `Sistema.Gerenciar` (PRODUTO, reservadas ao Administrador Sênior). Comprador recebe `Fornecedor.Criar`/`Fornecedor.Editar`; Aprovador recebe `Fornecedor.Aprovar`; Requisitante nasce sem nenhuma permissão do catálogo atual (módulo `Pedido.*` ainda sem enforcement/controller, GAP-01) — evolução documentada, não antecipada.
- **D7 — Classificação Produto × Negócio das configurações administrativas:** PRODUTO = configurações técnicas/corporativas que controlam a operação da plataforma (`UnidadeNegocio.Gerenciar`, `ConfiguracaoErp.Gerenciar`, `Sistema.Gerenciar` — este último cobre Identity Providers, Parâmetros, Feature Flags e Monitoramento). NEGÓCIO = configurações operacionais da própria Unidade de Negócio (as 8 permissões do item D6, mais Configuração de Notificações — remetente/canal específico da BU, apesar de hoje compartilhar o código `Sistema.Gerenciar` com Identity Providers). Esta divergência entre a classificação funcional de Configuração de Notificações (NEGÓCIO) e seu código de permissão atual (`Sistema.Gerenciar`, PRODUTO) é um gap documentado — ver Consequências.

**Consequências:**
- Os 5 controllers de path explícito passam a validar o alvo via `EscopoUnidadeNegocioPathFilter`; o achado do Gate anterior sobre eles está **resolvido**, não apenas registrado para decisão futura.
- `PerfisController` e `UsuariosController` passam a aceitar `unidadeNegocioId` opcional por query, honrado somente para escopo `Produto` (Administrador Sênior); os demais 10 controllers "BU da sessão" (Filiais, Centros de Custo, Unidades de Alocação, Configuração ERP, Parâmetros, Feature Flags, Monitoramento Operacional, Unidades de Negócio) não foram alterados nesta rodada por não haver necessidade funcional identificada — a arquitetura (D4) já suporta a extensão sem redesenho.
- `CatalogoInicialPerfisDeNegocioUseCase` garante idempotentemente o catálogo D5 em toda Unidade de Negócio, chamado por `ConcluirBootstrapUseCase` (BU nova do Bootstrap) e `CriarUnidadeNegocioUseCase` (BUs futuras); migration `AddCatalogoInicialPerfisDeNegocioO1Gate` faz o backfill idempotente nas BUs já existentes. Nunca cria ou toca em `Administrador Sênior`.
- A invariante de não-escalonamento de privilégio (ADR-0020) permanece a segunda barreira: um Administrador de BU não pode conceder a um novo Perfil uma permissão PRODUTO que ele mesmo não possui — mesmo que crie Perfis livremente na própria BU.
- **Gap documentado, não resolvido nesta rodada:** `Sistema.Gerenciar` continua sendo um único código de permissão compartilhado por 5 recursos de classificação distinta (D7) — Identity Providers/Parâmetros/Feature Flags/Monitoramento (PRODUTO) e Configuração de Notificações (NEGÓCIO). Dividir esse código em permissões mais granulares não foi feito por não ter sido solicitado e por exigir uma migration de dados de RBAC mais ampla — registrado como candidato de hardening futuro em `docs/audits/O1.14-InventarioDividasEGaps.md` (ver DEB-04).
- Testes de autorização cross-BU (6 cenários) em `EscopoAdministrativoUnidadeNegocioTests`; testes do catálogo inicial em `CatalogoInicialPerfisDeNegocioUseCaseTests`.
- **Achado de segurança identificado e corrigido durante esta própria implementação:** como `EscopoAdministrativo.Produto` era derivado apenas do nome do Perfil (`p.Nome == Perfil.AdministradorSenior`), um Administrador de BU com `Perfil.Gerenciar` poderia criar (ou renomear) um Perfil para esse nome na própria BU e, ao se vincular a ele, ganhar escopo Produto (cross-BU) sem nunca ter recebido essa concessão — escalonamento de **escopo**, não de permissão (a checagem de não-escalonamento de permissão do ADR-0020 não cobre este caminho, pois o ator não precisa conceder nenhuma permissão nova a si mesmo). Corrigido reservando "Administrador Sênior" em `CriarPerfilUseCase`/`AtualizarPerfilUseCase` (`RbacFalha.NomeReservado`) — nome só alcançável pelo Bootstrap, que nunca passa por esses casos de uso.
- `docs/audits/O1.14-InventarioDividasEGaps.md` e `.ai/PROJECT_STATE.md` atualizados; entregável #9 reclassificado para Concluído.

---

## ADR-0023: Arquitetura Canônica do Fluxo de Consulta/Cadastro de Fornecedor por CNPJ (Onda 2 — Frente Fornecedor/CNPJ)

**Status:** Aceito

**Contexto:** A auditoria técnica `docs/audits/Discovery-Fornecedor-CNPJ-Linx-Compras.md` e a modelagem arquitetural subsequente `docs/audits/Arquitetura-Fornecedor-CNPJ-Decisao.md` (que provisoriamente citava "ADR-0020" — numeração obsoleta, já ocupada por ADR-0020 de 09/08/2026; esta decisão assume corretamente o próximo número real disponível, **ADR-0023**, após ADR-0022) confirmaram, por leitura direta do código-fonte, quatro bugs reais na Enriquecimento Cadastral de Fornecedores por CNPJ (B2.2, concluída): (BUG-1) persistência automática do Fornecedor ao consultar, sem etapa de revisão sem escrita; (BUG-2) valor numérico bruto da situação cadastral do provider vazando até o `StatusBadge` do frontend; (BUG-3) qualquer erro de validação ou exceção não tratada sendo mascarado como "fonte indisponível"; (BUG-4) duas classes concorrentes de documento fiscal (`Cnpj.cs`, legado, e `DocumentoFiscal.cs`, o realmente usado), nenhuma validando dígito verificador nem normalizando máscara de forma confiável. O Product Owner decidiu formalizar a arquitetura de correção e abrir a primeira Work Order de implementação.

**Decisão:**

- **Arquitetura em camadas:** `Provider externo (BrasilAPI, substituível)` → `Adapter do Provider` → `Contrato Canônico de Consulta CNPJ (+Compras)` → `Application` → `Revisão humana explícita` → `Persistência +Compras` → `futuro Adapter Linx`. Provider externo não define domínio; BrasilAPI é implementação substituível. Visual Linx não define domínio canônico do +Compras — detalhes físicos do Linx (`CLIFOR`, `LX_SEQUENCIAL`, `CADASTRO_CLI_FOR`, filas ETL/WETL, vocabulário SAP/OBC/Redmine) pertencem exclusivamente a um futuro Adapter Linx, nunca ao domínio.
- **DocumentoFiscal — Value Object único:** eliminar a duplicidade conceitual entre `Cnpj.cs` e `DocumentoFiscal.cs`, mantendo `DocumentoFiscal` como único ponto de normalização: remoção de máscara (dígitos puros), validação de dígito verificador (módulo 11, CNPJ e CPF via `TipoPessoa`), igualdade por valor normalizado, máscara restrita à apresentação. `"15.436.940/0001-03"` e `"15436940000103"` devem produzir o mesmo valor.
- **Fluxo de consulta nunca persiste automaticamente:** `informar → normalizar → validar → consultar → revisar → confirmar → persistir`. Persistência ocorre exclusivamente por ação explícita do usuário ("Cadastrar fornecedor"/"Salvar"), nunca como efeito colateral da consulta.
- **Endereço:** apenas endereço principal no MVP — sem requisito real de múltiplos endereços identificado.
- **Contato:** DDD + telefone principal único no MVP — sem requisito real de múltiplos contatos identificado; segundo telefone da fonte é descartado conscientemente.
- **Situação cadastral:** contrato canônico interno (enum `Ativa, Baixada, Suspensa, Inapta, Nula, Desconhecida`), sem vazamento de valor numérico bruto do provider para o domínio ou para o frontend.
- **CNAE:** persistir apenas código + descrição do CNAE principal; CNAEs secundários não entram no domínio.
- **QSA:** não persistir QSA no +Compras. Justificativa: minimização de dados (LGPD, Art. 6º III), ausência de necessidade funcional comprovada, Linx investigado não possui QSA no cadastro mestre relevante. Consulta futura sob demanda, não armazenada, é opção em aberto caso Compliance a exija.
- **Proveniência híbrida:** dados normalizados + metadados da consulta (já existentes) + snapshot bruto/versionado do retorno do Provider (novo), com retenção de **180 dias**. Após 180 dias, o payload bruto pode/deve ser removido conforme mecanismo de expurgo seguro e testável a ser implementado; dados normalizados e metadados de proveniência permanecem. O snapshot bruto nunca é fonte de leitura do domínio nem substitui os dados normalizados — serve exclusivamente como evidência/auditoria.
- **Duplicidade:** mantida a regra canônica existente — `DocumentoFiscal` normalizado é único globalmente no +Compras, independentemente de `BusinessUnit` (já é o comportamento real do schema; vínculo com múltiplas unidades se dá pela identidade ERP, não pela duplicação da entidade Fornecedor).
- **Taxonomia de erros tipada:** `CnpjInvalido`, `NaoEncontrado`, `FonteIndisponivel`, `Timeout`, `LimiteDeConsultas`, `ErroDeAutenticacaoDoProvider`, `RespostaInvalida`, `ErroInterno` — como enum/hierarquia de exceptions tipadas no Application layer, nunca strings livres decidindo o comportamento.
- **Adapter Linx:** planejamento arquitetural autorizado. **Qualquer implementação de escrita real no Linx permanece bloqueada** até existir uma sessão específica de validação com especialista Visual Linx. O conhecimento hoje disponível (discovery + `docs/agents/ai-factory/temp/LinxKnowledge-Fornecedor-Discovery-Snapshot.md`) é a melhor evidência disponível, não o contrato oficial de escrita do Visual Linx. O gate futuro deve validar, entre outros: mecanismo oficial de cadastro, sequencial, duplicidade, triggers, procedures, filas ETL/WETL, múltiplos papéis de `CADASTRO_CLI_FOR`, efeitos colaterais, e o contrato `CADASTRO_CLI_FOR` → `FORNECEDORES`.

**Consequências:**
- Nenhum código, migration ou banco foi alterado por esta ADR — decisão exclusivamente arquitetural/de planejamento, formalizando `docs/audits/Arquitetura-Fornecedor-CNPJ-Decisao.md`.
- Corrigida a inconsistência documental da Work Order `B2.2-EnriquecimentoCadastralDeFornecedoresPorCnpj.md`: seu status interno ("Draft — não iniciada") divergia de `PROJECT_STATE.md`/`BACKLOG.md`, que já a tratavam como Concluída com evidência de commits, testes e migrations reais. Corrigido apenas o campo de status documental — histórico, escopo e evidências de B2.2 não foram alterados nem reexecutados.
- A sequência de Work Orders da frente Fornecedor/CNPJ foi decomposta como `B2.3` (DocumentoFiscal — normalização e dígito verificador) → `B2.4` (Contrato canônico + taxonomia de erros) → `B2.5`/`B2.6` (Situação cadastral canônica; State machine de revisão no frontend — podem correr em paralelo se as dependências reais permitirem) → `B2.7`/`B2.8` (Proveniência híbrida + retenção 180 dias; CNAE principal) → validação integrada do fluxo → `B2.9` (Adapter Linx, bloqueada), catalogadas em `.ai/BACKLOG.md` e detalhadas em `.ai/work-orders/backlog/`.
- Apenas `B2.3` foi aprovada e movida para `.ai/work-orders/active/`; as demais permanecem `Draft`/Planejadas em `.ai/work-orders/backlog/`, dependendo de nova aprovação explícita do Product Owner para ativação.
- `docs/audits/Arquitetura-Fornecedor-CNPJ-Decisao.md` e `docs/audits/Discovery-Fornecedor-CNPJ-Linx-Compras.md` permanecem preservados como evidência histórica desta decisão, sem alteração retroativa.

**Addendo B2.7 (12/08/2026) — formalização de detalhes de implementação não decididos na rodada original:**
- **Semântica exata de fronteira de expiração:** o payload bruto é elegível para expurgo somente quando `DataConsulta` é estritamente anterior a `ReferênciaUtc.AddDays(-180)` — ou seja, "retenção de 180 dias" significa preservado durante 180 dias completos; no dia exato do 180º aniversário da consulta o payload ainda é preservado, tornando-se elegível a partir do 181º dia. Coberto por testes de fronteira (179/180/181 dias).
- **Sanitização de QSA:** implementada como sanitizador explícito por Provider (`BrasilApiSnapshotSanitizer`, Infrastructure layer), não como motor de DLP genérico. Remove a chave `qsa` (case-insensitive) por completo do JSON antes de qualquer persistência, além de chaves defensivas de segredo (`token`, `authorization`, `senha`, `password`, `secret`, `apikey`, `api_key`, `chave`, `key`) — mesmo que a BrasilAPI não as retorne hoje, protege contra mudança futura de contrato do Provider. Um Provider futuro forneceria seu próprio sanitizador via `ICnpjConsultaProviderComSnapshot`, sem exigir alteração de domínio.
- **Permanência da parte estrutural:** confirmado por implementação — `Cnpj_Cpf`, `FonteConsulta`, `DataConsulta`, `Status`, `TipoErro`, `CorrelationId`, `BusinessUnit`, `ErpSistema` nunca são removidos ou alterados pelo mecanismo de expurgo; apenas `PayloadBrutoJson` é anulado. `TipoErroConsultaCnpj` — que a auditoria (seção L) já havia identificado como ausente do histórico — passou a ser persistido (como string, mesma convenção de `Status`), eliminando a necessidade de inferir o tipo de falha a partir de `MensagemErro`.
- **Limite de tamanho do snapshot:** 32.000 caracteres. Snapshot sanitizado que excede o limite é descartado por completo (nunca truncado de forma que produza JSON inválido) e sinalizado por `PayloadBrutoDescartadoPorTamanho` (metadado estrutural, sobrevive ao expurgo).
- **Desvio deliberado da Work Order de backlog:** `B2.7-ProvenienciaHibridaERetencao180Dias.md` (rascunho original) e a seção L da auditoria condicionavam a gravação do snapshot a "sucesso e criação/atualização real de um Fornecedor". Na implementação real, `FornecedorCnpjConsultaHistorico` já é gravado a cada consulta (sucesso ou falha), independentemente de qualquer persistência de `Fornecedor` — comportamento herdado e confirmado pela B2.6 (BUG-1: consultar nunca persiste Fornecedor). Gatear o snapshot a um evento de persistência posterior exigiria acoplar a gravação do histórico a um segundo momento do fluxo (`Persisting`), fora do escopo real desta sprint e sem correlação natural com o registro de consulta já existente. Optou-se por manter o snapshot atrelado ao mesmo evento em que o restante do histórico já é gravado (a própria consulta), sujeito à mesma sanitização e à mesma retenção de 180 dias — mantendo a garantia de que o snapshot nunca é fonte de leitura do domínio e nunca é gravado para tentativas sem corpo de resposta útil (ex.: timeout, CNPJ inválido).

## ADR-0024: Regra Transversal de Autoridade Linx ↔ +Compras — Linx Prevalece em Conflito/Ambiguidade

**Status:** Aceito

**Contexto:** Durante o discovery da B3 (Item Fiscal, `applications/mais-compras/docs/product/ContratoFuncionalPreliminar-B3-ItemFiscal.md`), foi comprovado tecnicamente (`docs/audits/Discovery-ItemFiscal-Pedido-EntradaFiscal-Consumiveis.md`) que `CADASTRO_ITEM_FISCAL.DATA_PARA_TRANSFERENCIA` é atualizado de forma incondicional por `DEFAULT GETDATE()` + trigger `LXUDT_CADASTRO_ITEM_FISCAL` em qualquer INSERT/UPDATE, servindo como timestamp confiável para resolução de conflito por "alteração mais recente" (Last Write Wins) entre Linx e +Compras — mesmo princípio já em produção para Fornecedores (`CADASTRO_CLI_FOR.DATA_PARA_TRANSFERENCIA`). Também foi comprovado que `ITEM_FISCAL_REF_FORNECEDOR` **não** possui timestamp próprio nem mecanismo (trigger ou fluxo VFP) que atualize a data do Item Fiscal pai — não existe, portanto, base técnica para um Last Write Wins confiável nessa coleção. Diante disso, o Product Owner decidiu generalizar um princípio de desempate para **qualquer** integração Linx ↔ +Compras, presente e futura, não só para a B3.

**Decisão:**

- **Regra geral de desempate:** sempre que houver conflito de dados, conflito de alterações, ambiguidade de autoridade, ou ausência de regra específica já aprovada definindo quem prevalece entre Linx e +Compras — **o Linx é o sistema mandatório; Linx prevalece.**
- **Esta regra é um fallback de último recurso, não substitui a exigência de matriz de autoridade por cadastro** já obrigatória em `applications/mais-compras/docs/architecture/CadFormFactory.md` §2 ("Nunca assuma sincronização simétrica por padrão" / cada cadastro precisa da sua própria matriz, levantada no discovery). A regra só se aplica quando o discovery específico do cadastro **não produziu** uma decisão determinística para o caso concreto — nunca como atalho para pular o discovery de autoridade por campo.
- **Aplicação concreta já decidida na B3 (exemplo de referência para futuras integrações):**
  - `CADASTRO_ITEM_FISCAL`: autoridade operacional compartilhada Linx + +Compras; quando há timestamps comparáveis, prevalece a alteração mais recente via `DATA_PARA_TRANSFERENCIA` (mecanismo comprovado). Se surgir situação ambígua que o Last Write Wins não resolva com segurança, Linx prevalece (aplicação direta desta ADR).
  - `ITEM_FISCAL_REF_FORNECEDOR`: sem timestamp confiável comprovado — não inventar mecanismo artificial de Last Write Wins; em divergência entre os dois sistemas, Linx prevalece como fonte mandatória de resolução. Isso não impede o usuário do +Compras de cadastrar/manter referências — apenas define o desempate quando os dois lados divergem sem outra regra determinística aplicável.
  - Inativação (já decidida, preservada e coerente com este princípio): Linx inativa → +Compras inativa; +Compras inativa localmente → não inativa o Linx.
- **Escopo:** transversal a qualquer integração Linx ↔ +Compras futura, não exclusivo da B3, salvo decisão funcional explícita do Product Owner determinando comportamento diferente para um caso específico.

**Consequências:**

- Nenhum código, tabela ou migration foi alterado — decisão exclusivamente de governança/arquitetura.
- `CadFormFactory.md` deve referenciar esta ADR na seção de matriz de autoridade (resolução de conflito), sem duplicar o texto da regra.
- `ContratoFuncionalPreliminar-B3-ItemFiscal.md` e o conhecimento persistido do Agent (`agents/knowledge/linx-item-fiscal-cadastro/`) referenciam esta ADR para a autoridade de Item Fiscal e de `ITEM_FISCAL_REF_FORNECEDOR`, em vez de duplicar a regra geral.
- Futuras integrações Linx ↔ +Compras (novos cadastros construídos pelo `CadFormFactory`) devem citar esta ADR ao definir sua própria matriz de autoridade, aplicando-a apenas como fallback de ambiguidade — nunca como substituto do discovery específico.
