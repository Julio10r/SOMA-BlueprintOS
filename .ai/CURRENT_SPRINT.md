...
**B2.3 concluída nesta mesma sessão.** `DocumentoFiscal` unificado como VO canônico (normalização por dígitos puros + dígito verificador módulo 11 de CNPJ/CPF); `Cnpj.cs` passa a delegar a ele; `ConsultaCnpjResultado.CriarFalha` corrigido para não revalidar documento já inválido. Achado durante a implementação: sincronização ERP de documento alfanumérico legado (Linx `CGC_CPF`) agora falha de forma controlada em vez de ser aceita silenciosamente — comportamento intencional pela ADR-0023 (compatibilidade com códigos legados é escopo do futuro Adapter Linx, B2.9), coberto por teste dedicado e registrado como risco em `.ai/PROJECT_STATE.md`. `dotnet build`/`dotnet test`: 737 unitários + 7 integração, 0 falhas; `dotnet ef migrations has-pending-model-changes`: nenhuma pendência, nenhuma migration criada. Nenhum contrato frontend foi tocado (sem impacto em TypeScript) — `tsc`/`npm test`/`npm run build` não eram necessários e não foram executados. Work Order movida para `.ai/work-orders/completed/`. **Adapter Linx (B2.9) NÃO foi implementado. Nenhuma escrita no SOMA_DESENV nesta Work Order.**

**B2.4 — Contrato Canônico de Consulta CNPJ + Taxonomia de Erros — CONCLUÍDA nesta mesma sessão** (12/08/2026). `TipoErroConsultaCnpj` (8 valores) introduzido no Application layer com `HttpStatusSugerido`/`PermiteRetry` derivados do tipo; `MensagemErro` passa a ser sempre derivada (nunca mais fonte de verdade). BUG-3 eliminado: CNPJ com dígito verificador inválido é rejeitado por `DocumentoFiscal.Create` sem chamar a BrasilAPI (`CnpjInvalido`); `BrasilApiCnpjProvider` classifica individualmente 404/429/401/403/5xx/payload nulo/`JsonException`/timeout/exceção inesperada, eliminando o `catch` genérico que colapsava tudo em "fonte indisponível". Endpoint `/consulta-cnpj` mantém `200 OK` em falha (decisão documentada — preserva o round-trip do mesmo objeto em outros dois endpoints), agora com `tipoErro`/`permiteRetry`/`httpStatusSugerido` no corpo. Contrato TypeScript atualizado, sem mudança de UI (fora do escopo B2.6). `dotnet build`/`dotnet test`: 744 unitários + 7 integração, 0 falhas; frontend `tsc -b`/`npm test` (116/116)/`npm run build` aprovados; `dotnet ef migrations has-pending-model-changes`: nenhuma pendência. Work Order movida para `.ai/work-orders/completed/`. **B2.5 NÃO foi iniciada.**

**B2.5 — Situação Cadastral Canônica — CONCLUÍDA nesta mesma sessão** (12/08/2026). Work Order movida `backlog/` → `active/` → `completed/`. Baseline confirmado antes de qualquer alteração: backend 744 unitários + 7 integração (0 falhas); frontend 116/116. Achado principal: nenhum `JsonStringEnumConverter` estava registrado no backend — todo enum do contrato HTTP serializava como inteiro bruto (causa raiz real do vazamento numérico até o `StatusBadge`), corrigido globalmente em `Program.cs`. `SituacaoCadastralCnpj` ampliado para `Ativa, Baixada, Suspensa, Inapta, Nula, Desconhecida`; campo tornado nullable (só preenchido em sucesso). Novo componente `SituacaoCadastralBadge.tsx` dedicado ao CNPJ; `StatusBadge` genérico (usado por ~15 telas de Administração) preservado sem alteração. Contrato TS sem `| string` residual. Persistência de `SituacaoCadastral`/`DataSituacaoCadastral` em `Fornecedor` (prevista no rascunho original da WO) foi deliberadamente fora do escopo real desta execução — nenhuma migration criada. `dotnet build`/`dotnet test`: 759 unitários + 7 integração, 0 falhas; frontend `tsc -b`/`npm test` (126/126)/`npm run build` aprovados; `dotnet ef migrations has-pending-model-changes`: nenhuma pendência. Work Order movida para `.ai/work-orders/completed/`. **B2.6 permanece Draft/Planejada, NÃO iniciada.**

**B2.6 — State Machine de Revisão no Frontend — CONCLUÍDA nesta mesma sessão** (12/08/2026). Work Order movida `backlog/` → `completed/`. Baseline confirmado: backend 759 unitários + 7 integração (0 falhas); frontend 126/126. Causa raiz real do BUG-1 investigada e confirmada: o backend (`ConsultarCnpjFornecedorUseCase`) já era somente leitura; o bug estava no frontend (`CadastroFornecedor.tsx` chamava `createSupplierDraft`/POST `/fornecedores` automaticamente sempre que a consulta não encontrava fornecedor existente, antes de qualquer revisão humana). Corrigido com state machine explícita `Idle → Validating → Consulting → Review → Persisting → Success` (+ estados de erro); persistência de Fornecedor novo passou a exigir clique explícito em "Cadastrar fornecedor" (novo componente `NovoFornecedorPanel.tsx`, com campos editáveis/complementáveis); inconsistência do bloqueio de situação cadastral corrigida (`Baixada/Suspensa/Inapta` em vez de só `Baixada`, em ambos os caminhos); proteção contra submissão duplicada adicionada (disabled + guarda por `useRef`). QSA, CNAE (B2.8) e proveniência (B2.7) não antecipados; nenhuma migration criada; zero escrita em `SOMA_DESENV` (todos os testes usam banco em memória/mocks). Smoke test via Chrome DevTools MCP não executado — impedimento documentado (exigiria backend real conectado a `SOMA_DESENV`, proibido). `dotnet build`/`dotnet test`: 760 unitários + 7 integração, 0 falhas; frontend `tsc -b`/`npm test` (130/130)/`npm run build` aprovados; `dotnet ef migrations has-pending-model-changes`: nenhuma pendência. **B2.7 permanece Draft/Planejada, NÃO iniciada.**

**B2.7 — Proveniência Híbrida da Consulta CNPJ + Snapshot Bruto com Retenção de 180 Dias — CONCLUÍDA nesta mesma sessão** (12/08/2026). Work Order movida `backlog/fase-b/` → `active/` → `completed/`. Baseline confirmado antes de qualquer alteração: backend 760 unitários + 7 integração (0 falhas); `dotnet ef migrations has-pending-model-changes`: nenhuma pendência.

Modelo (Domain, `FornecedorCnpjConsultaHistorico`): três campos novos — `TipoErro` (`TipoErroConsultaCnpjHistorico?`, enum espelho do `TipoErroConsultaCnpj` do Application layer, já que Domain não referencia Application; persistido como string, mesma convenção de `Status`; a auditoria já apontava sua ausência), `PayloadBrutoJson` (`string?`, snapshot bruto sanitizado, limite de `LimitePayloadBrutoCaracteres = 32_000` caracteres validado no construtor) e `PayloadBrutoDescartadoPorTamanho` (`bool`). Novo método `PayloadBrutoExpirado(DateTimeOffset referenciaUtc)` define a semântica exata de fronteira de retenção: elegível para expurgo apenas quando `DataConsulta < referenciaUtc.AddDays(-180)` — preservado aos 179 dias e **exatamente aos 180 dias**; elegível a partir do 181º dia. Método `ExpirarPayloadBruto()` (idempotente, só anula `PayloadBrutoJson`).

Captura na fronteira Provider→Adapter: nova interface opcional `ICnpjConsultaProviderComSnapshot : ICnpjConsultaProvider` (Application) com `ConsultarComSnapshotAsync` retornando `CnpjConsultaProviderResposta(Resultado, SnapshotBrutoSanitizado, SnapshotDescartadoPorTamanho)`. **`ICnpjConsultaProvider.ConsultarAsync` não foi alterado** — decisão deliberada para preservar os ~20 testes pré-existentes de `BrasilApiCnpjProviderTests`. `BrasilApiCnpjProvider` implementa ambas (delegando a uma implementação interna comum), lê o corpo HTTP bruto via `ReadAsStringAsync` e o repassa para sanitização; nenhum campo `BrasilApiCampoX` foi adicionado ao schema — o snapshot é opaco e tratado como texto, não como schema do provider.

Sanitização (`BrasilApiSnapshotSanitizer`, Infrastructure, específico da BrasilAPI — não é DLP genérico): remove a chave `qsa` (case-insensitive) por completo do JSON — regra absoluta da ADR-0023, verificada com fixture sintética de sócios falsos — além de chaves defensivas de segredo (`token`, `authorization`, `senha`, `password`, `secret`, `apikey`, `api_key`, `chave`, `key`), mesmo que a BrasilAPI não as retorne hoje. Corpo nulo/vazio/não-JSON/não-objeto nunca é persistido (retorna `null`, nunca lança). Corpo sanitizado que excede o limite de 32.000 caracteres é descartado por completo (nunca truncado de forma insegura) e `SnapshotDescartadoPorTamanho=true`. Regras por cenário de falha: `CnpjInvalido` e falhas de rede/timeout (`Timeout`, `FonteIndisponivel` por exceção) → sempre snapshot nulo (nunca há corpo); `NotFound`/`TooManyRequests`/`RespostaInvalida` (corpo malformado em 2xx) → snapshot sanitizado quando há corpo útil; `Unauthorized`/`Forbidden` (falha de autenticação da própria integração, não do CNPJ consultado) → sempre snapshot nulo, por ser diagnóstico de infraestrutura.

`ConsultarCnpjFornecedorUseCase`: detecta via type-check (`provider is ICnpjConsultaProviderComSnapshot`) se o Provider suporta snapshot; se não suportar, grava `PayloadBrutoJson=null` sem qualquer alteração de domínio (compatibilidade com Providers futuros). Mapeamento 1:1 por nome entre `TipoErroConsultaCnpj` (Application) e `TipoErroConsultaCnpjHistorico` (Domain) via `Enum.Parse`.

Retenção/expurgo: `IFornecedorCnpjConsultaHistoricoRepository.ExpurgarPayloadBrutoExpiradoAsync(DateTimeOffset referenciaUtc)` usa EF Core `ExecuteUpdateAsync` (batch update, sem carregar snapshots em memória, sem desserializar JSON) quando o provider é relacional; fallback por rastreamento de entidades (mesmo filtro, mesma idempotência) no provider InMemory usado pelos testes, já que `ExecuteUpdateAsync` não é traduzido pelo InMemory. `ExpurgarPayloadBrutoConsultaCnpjUseCase` (Application) expõe a operação, registrada em `ServiceCollectionExtensions` — pronta para invocação futura por agendador; **nenhum `BackgroundService`/Hangfire/Quartz foi introduzido** (nenhum mecanismo de job pré-existia no projeto). Idempotência comprovada por teste (executar duas vezes: primeira expurga, segunda retorna 0 sem erro); nunca remove `FornecedorCnpjConsultaHistorico`/`Fornecedor`; nunca altera `TipoErro`/`FonteConsulta`/`DataConsulta`/`Status`/etc.; afeta apenas registros elegíveis (testado com mistura de registros recentes/expirados na mesma tabela).

Índice: `HasIndex(x => x.DataConsulta)` adicionado — justificado porque o expurgo varre a tabela inteira periodicamente e precisa localizar eficientemente os registros elegíveis sem full scan; nenhum índice sobre `PayloadBrutoJson` (nenhuma leitura operacional filtra por seu conteúdo).

Migration `20260812065045_AddProvenienciaHibridaConsultaCnpjB27`: `AddColumn PayloadBrutoDescartadoPorTamanho bit NOT NULL DEFAULT 0`, `AddColumn PayloadBrutoJson nvarchar(max) NULL`, `AddColumn TipoErro nvarchar(40) NULL`, `CreateIndex IX_FornecedoresCnpjConsultas_DataConsulta` — revisada manualmente: sem drops, sem alteração de outras tabelas, sem backfill sintético de payload para registros antigos. `dotnet ef migrations has-pending-model-changes` confirmou ausência de pendência após a migration.

LGPD/segurança: QSA nunca persistido (mesmo dentro do snapshot); nenhum segredo/header de autenticação persistido; snapshot nunca é lido pelo domínio (contrato canônico `ConsultaCnpjResultado` não referencia JSON externo); nenhum endpoint/DTO de `BlueprintOS.Api` referencia `PayloadBrutoJson`/`FornecedorCnpjConsultaHistorico` (confirmado por grep — snapshot nunca chega ao frontend); nenhum endpoint de leitura de payload bruto foi criado (fora do escopo desta sprint); log do expurgo registra apenas contagem, nunca conteúdo de payload.

**Desvio de escopo documentado** (ADR-0023, addendo B2.7, e na própria Work Order `B2.7-ProvenienciaHibridaERetencao180Dias.md`): o rascunho original e a seção L da auditoria condicionavam a gravação do snapshot a "sucesso com criação/atualização real de um Fornecedor". A implementação real manteve o snapshot atrelado ao mesmo evento em que o restante do histórico já é gravado — a cada consulta, sucesso ou falha, independentemente de persistência de `Fornecedor` (comportamento herdado e confirmado pela B2.6/BUG-1: consultar nunca persiste Fornecedor). Gatear o snapshot a um evento de persistência posterior exigiria um segundo ponto de gravação fora do fluxo de consulta, sem correlação natural com o histórico já existente, e estava fora do escopo real desta sprint.

Testes novos (43): `FornecedorCnpjConsultaHistoricoTests` (fronteiras 179/180/181 dias, idempotência de `ExpirarPayloadBruto`, limite de tamanho no construtor, `TipoErro` nulo em sucesso); `BrasilApiSnapshotSanitizerTests` (remoção de QSA case-insensitive com fixture sintética, remoção de chaves de segredo, corpo nulo/malformado/não-objeto, limite de tamanho, preservação de campos cadastrais); extensões em `BrasilApiCnpjProviderTests` (snapshot sanitizado sem QSA no sucesso, snapshot nulo em timeout/CnpjInvalido/erro de autenticação, snapshot sanitizado em 404 com corpo útil, snapshot nulo em corpo malformado, descarte por tamanho com flag, paridade do contrato canônico entre `ConsultarAsync`/`ConsultarComSnapshotAsync`); extensões em `ConsultarCnpjFornecedorUseCaseTests` (persistência de snapshot quando o Provider suporta, snapshot nulo quando não suporta, ausência de QSA no snapshot persistido, `TipoErro` persistido em falha tipada, `TipoErro` nulo em sucesso, flag de descarte por tamanho persistida); `FornecedorCnpjConsultaHistoricoRepositoryTests` (fronteiras de retenção, idempotência, registro já nulo não é tocado, apenas registros elegíveis são modificados em tabela mista, nunca deleta linhas/`Fornecedor`); `ExpurgarPayloadBrutoConsultaCnpjUseCaseTests` (delegação ao repositório, referência UTC, segurança de execução repetida). Regressão B2.4/B2.5/B2.6 reverificada sem alteração: CNPJ inválido continua sem chamada HTTP, taxonomia de erro correta, situação cadastral canônica preservada, JSON externo encapsulado no Adapter, e o teste `Execute_Should_Never_Persist_A_Fornecedor_As_Side_Effect_Of_A_Query` (B2.6) continua verde sem modificação.

`dotnet build`/`dotnet test`: **803 unitários + 7 integração, 0 falhas** (baseline B2.6: 760 unitários; +43 novos); `dotnet ef migrations has-pending-model-changes`: nenhuma pendência após a migration criada. Frontend **não tocado** — B2.7 não exigiu mudança de contrato público; baseline 130/130 preservado sem nova execução de `tsc -b`/`npm test`/`npm run build`. Work Order movida `active/` → `.ai/work-orders/completed/`. **B2.8 permanece Draft/Planejada, NÃO iniciada. B2.9 permanece bloqueada.**


**B2.8 — CNAE Principal no Modelo Persistente de Fornecedor — CONCLUÍDA nesta mesma sessão** (12/08/2026). Work Order movida `backlog/fase-b/` → `active/` → `completed/`. Baseline confirmado antes de qualquer alteração: backend 803 unitários + 7 integração (0 falhas); frontend 130/130; `dotnet ef migrations has-pending-model-changes`: nenhuma pendência.

Estado real do CNAE antes desta Work Order (confirmado por leitura de código, grep de "cnae" em todo o repositório — zero hits em `.cs`/`.ts`/`.tsx`): nenhum campo de CNAE existia em `Fornecedor`, em `ConsultaCnpjResultado` (contrato canônico) ou em qualquer DTO/frontend. O `BrasilApiCnpjProvider` já descartava `cnae_fiscal`/`cnae_fiscal_descricao`/`cnaes_secundarios` silenciosamente, por nunca declará-los no record de desserialização `BrasilApiCnpjResponse` — não havia código tentando sincronizar CNAE com Linx, nem estrutura de CNAEs secundários (`FornecedorCnae`/`CnaeSecundario`) em nenhum lugar do domínio.

Modelagem (Domain, `Fornecedor`): dois campos novos, `CnaePrincipalCodigo` (`string?`, dígitos puros) e `CnaePrincipalDescricao` (`string?`, texto livre da fonte) — nomenclatura confirmada em `docs/audits/Arquitetura-Fornecedor-CNPJ-Decisao.md` (seção H). Nenhuma entidade/tabela CNAE, coleção ou catálogo nacional foi criada. Normalização centralizada em `Fornecedor.NormalizarCnaeCodigo` (dígitos puros via `char.IsDigit`), reutilizada pelo contrato canônico (`ConsultaCnpjResultado.CriarSucesso`) e pela aplicação de enriquecimento (`AplicarEnriquecimentoCnpj`) — evita `Replace`/regex espalhados pelo código. Novo método `Fornecedor.DefinirCnaePrincipal(codigo, descricao, alteradoEm)`, chamado exclusivamente pelas operações explícitas de cadastro (`CadastrarFornecedorUseCase`) e atualização (`AtualizarFornecedorUseCase`) — nunca a partir de uma consulta isolada. Ambos os campos são opcionais: cadastro/atualização funcionam normalmente sem CNAE informado (regressão de cadastro manual pós-falha do Provider, B2.6, preservada).

Contrato canônico (`ConsultaCnpjResultado`, Application): `CnaePrincipalCodigo`/`CnaePrincipalDescricao` adicionados ao record; `CriarSucesso` ganhou os dois parâmetros opcionais (normalizados na fronteira); `CriarFalha` foi reescrito com argumentos nomeados (em vez de posicionais) especificamente para eliminar o risco de desalinhamento ao adicionar novos campos ao record — um erro de contagem posicional real ocorreu durante a implementação e foi corrigido antes do commit. Falha de consulta nunca carrega CNAE (mesma regra já aplicada à situação cadastral desde a B2.5).

Provider (`BrasilApiCnpjProvider`): `BrasilApiCnpjResponse` ganhou `cnae_fiscal` (`int?` — a BrasilAPI já entrega o código como inteiro, portanto dígitos puros por natureza, sem necessidade de mascaramento a remover) e `cnae_fiscal_descricao` (`string?`). Novo record privado `BrasilApiCnaeSecundarioItem` desserializa `cnaes_secundarios` apenas para não corromper o parsing do restante do payload — a lista nunca é lida nem mapeada para o contrato canônico; teste explícito (`ConsultarAsync_Should_Discard_Cnaes_Secundarios_At_The_Adapter_Boundary`) envia um payload com dois CNAEs secundários e comprova, via reflexão sobre `ConsultaCnpjResultado`, que nenhuma propriedade de secundários existe no tipo e que apenas o principal é mapeado. Situação cadastral e taxonomia de erros (B2.4/B2.5) não foram alteradas por esta mudança — confirmado pela suíte completa do Provider permanecendo verde.

Review (B2.6, `SupplierComparison.tsx`): novo bloco "CNAE principal" com `Código`/`Descrição`, exibido sempre que a consulta tiver sucesso (mesmo padrão dos demais blocos de dados retornados). Código apresentado com máscara `6201-5/01` via `formatCnaeCodigo` (função pura, só regex de apresentação — nunca a representação persistida); ausência de CNAE renderiza "Não informado" sem quebrar a tela (testado). Editabilidade: **decisão confirmada de não tornar o CNAE editável no formulário de novo fornecedor** — a seção H da auditoria já apontava que nenhuma das 5 implementações Linx lidas no discovery preenche CNAE manualmente, e o dado vem de fonte oficial já revisada em tela; não há requisito formal exigindo edição, então `NovoFornecedorPanel.tsx` não foi alterado. `supplierEnrichmentApi.ts` (`createSupplierDraft`) passou a incluir `cnaePrincipalCodigo`/`cnaePrincipalDescricao` no corpo do POST, lidos diretamente do objeto `consulta` já exibido na Review — nunca durante a consulta em si (testado: nenhum POST em `/fornecedores` ocorre antes do clique explícito em "Cadastrar fornecedor"; o POST, quando ocorre, carrega o CNAE).

Fornecedor existente (`FornecedorEnriquecimentoComparer`, fluxo B2.2/B2.6 de comparação campo a campo): `CnaePrincipalCodigo`/`CnaePrincipalDescricao` adicionados à mesma lista de campos comparados (`Add(divergencias, nameof(Fornecedor.CnaePrincipalCodigo), ...)`) — uma divergência de CNAE é calculada exatamente como qualquer outro campo (RazaoSocial, Email, etc.), nunca aplicada ao Fornecedor sem a aprovação explícita já exigida por `AprovarEnriquecimentoFornecedorUseCase`. Teste dedicado (`Analyze_Should_Create_Cnae_Divergence_But_Never_Update_Fornecedor_Automatically`) comprova que a análise calcula a divergência sem tocar o Fornecedor persistido; outro teste (`Approve_Should_Persist_Cnae_Principal_Only_After_Explicit_Approval`) comprova a persistência apenas após aprovação. Princípio absoluto preservado: consulta não altera Fornecedor existente automaticamente.

API (`FornecedorRequest`/`AtualizarFornecedorRequest`, `BlueprintOS.Api`): dois campos opcionais adicionados, propagados aos DTOs de Application (`CadastrarFornecedorDto`/`AtualizarFornecedorDto`/`FornecedorDto`) sem breaking change (todos os campos novos são opcionais/nullable, posição final dos records). QSA e secundários confirmados fora de qualquer contrato tocado — nenhum endpoint novo, nenhuma exposição de `PayloadBrutoJson`/snapshot B2.7 (não referenciado por esta Work Order).

Migration `20260812071704_AddCnaePrincipalFornecedorB28`: `AddColumn CnaePrincipalCodigo nvarchar(7) NULL`, `AddColumn CnaePrincipalDescricao nvarchar(300) NULL` na tabela `Fornecedores` — revisada manualmente: sem drops, sem alteração de outras tabelas, sem backfill de fornecedores históricos (permanecem válidos com CNAE nulo). `dotnet ef migrations has-pending-model-changes` confirmou ausência de pendência após a migration.

Testes novos (10): backend (+7) — `BrasilApiCnpjProviderTests` (mapeamento correto de código/descrição, ausência de CNAE não quebra consulta, secundários descartados na fronteira com prova por reflexão); `FornecedorUseCasesTests` (CNAE persistido quando informado com normalização de máscara, cadastro funciona sem CNAE); `FornecedorEnriquecimentoUseCasesTests` (divergência calculada sem alteração automática do Fornecedor, persistência apenas após aprovação explícita). Frontend (+3) — `CadastroFornecedor.test.tsx` (Review exibe CNAE mascarado + descrição; ausência de CNAE não quebra a Review; CNAE só é enviado no POST de criação após confirmação explícita). Regressão B2.3–B2.7 reverificada sem alteração: DocumentoFiscal/dígito verificador, taxonomia de erros, situação cadastral canônica, state machine de Review (BUG-1), proveniência híbrida/snapshot B2.7 — todos os testes pré-existentes permanecem verdes sem modificação.

Smoke test via Chrome DevTools MCP: **não executado** — impedimento documentado, idêntico ao já registrado na B2.6 (exigiria backend real conectado a um banco de desenvolvimento do +Compras isolado e seguro para escrita, que não está disponível nesta sessão; `SOMA_DESENV`/Linx permanecem com escrita zero). Cobertura validada integralmente por testes automatizados com banco em memória/mocks (backend) e mocks de `fetch` (frontend).

`dotnet build`/`dotnet test`: **810 unitários + 7 integração, 0 falhas** (baseline B2.7: 803 unitários; +7 novos); `dotnet ef migrations has-pending-model-changes`: nenhuma pendência após a migration criada. Frontend `tsc -b`/`npm test` (**133/133**, baseline 130 + 3 novos)/`npm run build` aprovados. Zero escrita em `SOMA_DESENV`/Linx. Adapter Linx (B2.9) **NÃO foi implementado**. Work Order movida `backlog/fase-b/` → `active/` → `.ai/work-orders/completed/`. **B2.9 permanece Draft/Planejada — Bloqueada, NÃO iniciada.**

---

## B2.9 — Adapter Linx (Fornecedor/CNPJ) — Implementação (12/08/2026) — Em Desenvolvimento, NÃO concluída

Gate Pré-B2.9 já desbloqueado (sessões anteriores); PO autorizou explicitamente o início da implementação nesta sessão, incluindo testes reais controlados contra `SOMA_DESENV` (banco de desenvolvimento). Work Order movida `backlog/fase-b/` → `active/`.

Implementado: contrato `IGarantirFornecedorErpAdapter`/`GarantirFornecedorErpRequest`/`GarantirFornecedorErpResultado` (Application), Adapter real `SomaGarantirFornecedorErpAdapter` (Infrastructure) com a semântica "garantir/atualizar fornecedor no ERP" — reconsulta obrigatória por CNPJ com `WITH (UPDLOCK, HOLDLOCK)` dentro de uma única transação curta, decisão CREATE/ADD_ROLE/UPDATE, `LX_SEQUENCIAL` para novo `CLIFOR`, sanitização de `NOME_CLIFOR` (validada contra a trigger real `LXI_ANM_CADASTRO_CLI_FOR`), `INDICA_FORNECEDOR` sempre tratado, nunca cria `FILIAIS`/`CLIENTES_ATACADO`, rollback total em qualquer falha. Taxonomia de erros nova (`ErpFornecedorErro`/`ErpFornecedorEscritaException`) sem vazamento de SQL/stack trace. Novo use case `GarantirFornecedorNoErpUseCase` e endpoint `POST /api/fornecedores/{id}/garantir-erp` (RBAC `Fornecedor.Editar` — pendente confirmação do PO). DI registrado.

Testes: 5 novos unitários (`GarantirFornecedorNoErpUseCaseTests`) + 2 novos de integração real (`GarantirFornecedorNoErpIntegrationTests`, atrás de opt-in explícito `B29_REAL_WRITE_TESTS=1`, nunca habilitado em CI). **810 → 815 unitários**, **7 → 9 integração**, 0 falhas. `dotnet ef migrations has-pending-model-changes`: nenhuma pendência (nenhuma migration criada, nenhum schema Linx alterado).

Validação real em `SOMA_DESENV` (12/08/2026, CNPJs sintéticos reservados, prefixo `91999000000xx`): CREATE e convergência CREATE→UPDATE confirmados funcionando (LX_SEQUENCIAL retornou CLIFORs reais 315506/315507); trigger de sanitização de nome confirmada compatível. **Pendências que impedem a conclusão da B2.9 nesta sessão**: (1) dois registros de teste (`COD_CLIFOR` 315506 e 315507) não puderam ser removidos — o `DELETE` de limpeza travou repetidamente (60s e 90s) sem diagnóstico possível (conta de aplicação sem `VIEW SERVER STATE`), permanecendo em `SOMA_DESENV` com `INDICA_FORNECEDOR = 1` e sem linha em `FORNECEDORES`; (2) em um dos cenários reais, o INSERT em `FORNECEDORES` não persistiu sem exceção conclusiva capturada — requer investigação adicional; (3) confirmação do PO sobre o nível de RBAC do endpoint; (4) ausência de confirmação de índice único em `CADASTRO_CLI_FOR.CGC_CPF` (mitigação de concorrência CREATE/CREATE é parcial, não uma garantia). Revisão de segurança independente executada nesta sessão: sem achados de SQL injection ou vazamento de dado sensível.

**B2.9 permanece `Em Desenvolvimento` em `active/`, NÃO movida para `completed/`.** Nenhum commit ou push realizado nesta sessão — aguardando revisão do PO. Detalhe completo no relatório final da própria Work Order (`.ai/work-orders/active/B2.9-AdapterLinxFornecedorCnpj.md`).

---

## B2.9 — Rodada de Diagnóstico Técnico Controlado (12/08/2026) — 4 pendências resolvidas com evidência real

PO aprovou `Fornecedor.Editar` como permissão definitiva do endpoint (sem criar permissão nova) e autorizou acesso/escrita controlada adicional em `SOMA_DESENV` para diagnóstico.

**Achado 1 — órfãos e "INSERT que não persistia" não eram bugs do Adapter.** Eram dois bugs no próprio harness de teste de integração real: (a) um `finally` de limpeza que relançava exceção mascarava o resultado real da asserção original (clássico exception-in-finally); (b) a limpeza rodava dois `DELETE` sem transação explícita — o primeiro (FORNECEDORES, rápido) autocommitava sozinho, o segundo (CADASTRO_CLI_FOR, lento por cascata de ~11 triggers de auditoria/ETL, **medido em ~2min40s por execução, não bloqueio**) estourava o timeout do cliente, deixando exatamente o padrão de órfão relatado. Ambos corrigidos (limpeza agora transacional, timeout de 200s). Reexecutado CREATE/Convergência com o harness corrigido: **FORNECEDORES sempre persistiu corretamente** — a falha nunca existiu no Adapter.

**Achado 2 — bug real encontrado e corrigido: FK de ADD_ROLE.** Descoberta nova (schema real, `sys.foreign_keys`): `FORNECEDORES.FORNECEDOR` tem FK física para `CADASTRO_CLI_FOR.NOME_CLIFOR` (`XFK12594_FORNECEDORES`, não documentada em nenhum discovery anterior). O caminho ADD_ROLE recalculava esse valor a partir do nome da chamada atual em vez de reutilizar o `NOME_CLIFOR` já gravado — falhava sempre que o nome não fosse idêntico ao já cadastrado. Nunca afetou CREATE nem UPDATE. Corrigido (`ReconsultarComLockAsync` agora lê `NOME_CLIFOR` da linha travada; `AdicionarPapelFornecedorAsync` reusa esse valor) e validado com teste real: cadastro Cliente pré-existente + nome diferente na chamada → ADD_ROLE bem-sucedido, papel Cliente preservado, nome original mantido.

**Achado 3 — atomicidade provada com falha real.** Teste real provocou uma violação de PK genuína no SQL Server (`Number=2627`, mensagem completa capturada) e confirmou rollback total — nenhum registro do CNPJ de teste permaneceu. Código revisado: única `SqlConnection`/`SqlTransaction` por chamada, todos os comandos (incl. `LX_SEQUENCIAL`) vinculados a ela, rollback em qualquer exceção.

**Achado 4 — concorrência CREATE/CREATE real validada.** Duas chamadas simultâneas para o mesmo CNPJ sintético contra SOMA_DESENV real resultaram em exatamente 1 `CADASTRO_CLI_FOR` + 1 `FORNECEDORES`, sem duplicidade. Ressalva honesta: `CADASTRO_CLI_FOR.CGC_CPF` não tem índice único físico (confirmado via `sys.indexes`) — proteção é aplicacional (`UPDLOCK, HOLDLOCK`), validada empiricamente, não uma garantia física absoluta.

**Limpeza:** os 4 registros de teste então existentes em `SOMA_DESENV` (`COD_CLIFOR` 315506-315509) foram removidos com sucesso (timeout generoso de ~3min por operação, sem envolver `VIEW SERVER STATE`). **Zero resíduo de teste ao final desta rodada.**

**Testes:** 815 unitários (inalterado) + 12 integração (9 → 12, todos atrás de opt-in `B29_REAL_WRITE_TESTS=1`, nunca em CI), todos aprovados. `dotnet build`: 0 erros/warnings novos. `dotnet ef migrations has-pending-model-changes`: nenhuma pendência. Revisão de segurança pós-correção: sem achados de SQL injection/vazamento/UPDATE ou DELETE sem filtro.

**B2.9 permanece `Em Desenvolvimento` em `active/`, NÃO concluída.** Único item que ainda impede o fechamento formal: validação manual pelo Visual Linx real (fora do alcance desta sessão, depende do acesso desktop do PO). Nenhum commit ou push realizado nesta sessão. Detalhe completo (16 seções) no relatório desta rodada, anexado ao final de `.ai/work-orders/active/B2.9-AdapterLinxFornecedorCnpj.md`.

---

## B2.9 — Fechamento Técnico (17/08/2026) — IMPLEMENTAÇÃO TÉCNICA CONCLUÍDA; HOMOLOGAÇÃO MANUAL TRANSFERIDA PARA O GATE FINAL DA ONDA 1

Decisão do Product Owner: postergar a homologação manual da B2.9 (validação pelo Visual Linx real), incorporando-a ao **Gate Final de Homologação Manual da Onda 1**, que só ocorrerá após implementações restantes da Onda 1, Design Review consolidado, correções decorrentes do Design Review e regressão automatizada final.

Reconciliação técnica executada nesta sessão (sem qualquer alteração de código de produção): `dotnet build BlueprintOS.sln` — 0 erros/0 warnings; `dotnet test BlueprintOS.sln` — **823 unitários + 13 integração, 0 falhas** (todos os testes reais de escrita de B2.9 seguem atrás do opt-in `B29_REAL_WRITE_TESTS=1`, não executados nesta sessão por ausência de acesso à VPN corporativa/`SOMA_DESENV` neste ambiente — comportamento já coberto por evidência real de rodadas anteriores da própria Work Order); `dotnet ef migrations has-pending-model-changes` — nenhuma pendência. Frontend: `tsc -b` sem erros; `npm test` — **23 arquivos, 140 testes aprovados**; `npm run build` — build de produção aprovado. Investigação de DELETE funcional: `ExcluirFornecedorUseCase` opera apenas sobre a tabela local de rascunho `Fornecedores` (pré-ERP, por `TemporaryUserId`), código pré-existente não tocado por esta WO e fora do caminho do Adapter — o Adapter nunca executa DELETE contra o ERP Linx. Nenhum bloqueador técnico identificado.

**B2.9 movida de `active/` para `completed/`** com status **"Concluída — Implementação Técnica"**. Homologação manual pelo Visual Linx real permanece **PENDENTE**, formalmente transferida ao Gate Final da Onda 1 (que deverá cobrir também autenticação, sessão, RBAC, perfis, usuários, filiais, centros de custo, fornecedores, consulta CNPJ, Review, CREATE/UPDATE/sincronização ERP, retry, permissões, navegação e CRUDs principais). **A Onda 1 não é declarada concluída por este fechamento; a Onda 2 não é iniciada; nenhuma nova Work Order é aberta.**

---

## DR-PosOnda1 — Correções Consolidadas do Design Review Pós-Onda 1 (17/08/2026) — CONCLUÍDA TECNICAMENTE

Execução consolidada dos 8 lotes (DR.1–DR.8) propostos em `docs/architecture/Design-Review-Consolidado-Pos-Onda1.md`, tratados como fases internas de uma única frente (nenhuma Work Order fragmentada, nenhuma Onda 2/B3 iniciada). Work Order: `.ai/work-orders/completed/DR-PosOnda1-CorrecoesConsolidadas.md`.

Os três achados P1 que bloqueavam o Gate foram resolvidos: **DR-06** (overflow de página inteira em 1024×768, corrigido com scroll interno estrutural nas tabelas, não `overflow-x:hidden`); **DR-10** (painel de decisão de Fornecedor aparecendo sem nenhum dado visível quando a reconsulta externa falha para um fornecedor já existente, corrigido com um snapshot dos dados já cadastrados); **DR-18** (`ExcluirFornecedorUseCase` executava `DbSet.Remove` físico contra a tabela real `Fornecedores` — não era apenas rascunho pré-ERP como uma nota anterior desta sprint registrou; corrigido reaproveitando `Fornecedor.Status`/`AlterarStatus`, já existente no domínio e já usado pela sincronização ERP; classe renomeada para `InativarFornecedorUseCase`, rota HTTP `DELETE /fornecedores/{id}` inalterada).
...
## B3 — Bloco 5A concluído, Multi-BU normalizado e Gate B3 aprovado (04/09/2026) — ajuste documental autorizado pelo Product Owner

**Natureza:** esta entrada reconcilia `CURRENT_SPRINT.md` (sem registro desde 02/09/2026) com o trabalho técnico já implementado, testado e publicado em `origin/main` (commit `d8dac6ed82aa0cfaa2222b4c56b6288fbc241a77`, "feat: complete Onda 2 B3 integration and multi-BU foundation") e com o registro cronológico completo em `applications/mais-compras/docs/cadernos/Onda-2.md` ("ENCERRAMENTO FORMAL DA ONDA 2", 04/09/2026). Nenhum código, migration, teste ou script foi alterado por este ajuste.

**Bloco 5A (sincronização Linx → +Compras) CONCLUÍDO dentro do escopo aprovado:** RAW/REFINED/reconciliação implementados e certificados para Fornecedor, Item Fiscal, Cadastro de Apoio (Conta Contábil/Unidade de Medida/Centro de Custo/Filial), Fornecedor Domínios e Item Fiscal Referência Fornecedor. Auditoria RAW determinística cobriu os 5 consumidores RAW→REFINED do B3 (Fornecedor já corrigido na bateria final; Cadastro de Apoio, Item Fiscal, Fornecedor Domínios e Item Fiscal Referência Fornecedor corrigidos nesta auditoria, cada um com critério de desempate próprio respeitando os dados reais do dataset).

**Multi-BU normalizado para o escopo atual (Grupo Soma):** `Fornecedor`/`FornecedorLinxVinculo` (identidade `UnidadeNegocioId + Cnpj_Cpf`), `IntegrationOccurrence` e `LinxDatasetLoadState` (nos 3 pipelines REFINED que o consomem diretamente) passam a ter dimensão de Unidade de Negócio. 4 migrations aplicadas com sucesso em `MAISCOMPRAS Development` (`NormalizarMetadadosApoioPorUnidadeNegocioOnda2`, `NormalizarFornecedorPorUnidadeNegocioOnda2`, `NormalizarIntegrationOccurrencePorUnidadeNegocioOnda2`, `NormalizarLinxDatasetLoadStatePorUnidadeNegocioOnda2`) — zero perda de dado (contagens de linhas idênticas antes/depois: Fornecedores 73.449, FornecedorLinxVinculos 76.419, IntegrationOccurrences 9.667, LinxDatasetLoadState 5), backfill 100% correto, `dotnet ef migrations has-pending-model-changes` sem pendências contra o banco real.

**Gate Técnico B3 (Bloco 6): APROVADO (04/09/2026).** Bateria final de certificação contra `SOMA_DESENV`/`MAISCOMPRAS Development`: incremental normal PASS nos 5 datasets baselined (Fornecedores, Filiais, Centros de Custo, Contas Contábeis, Itens Fiscais); teste controlado real e reversível de ~101 Fornecedores (INATIVO=1 → detecção → reconciliação → restauração INATIVO=0 → detecção → reconciliação) PASS; idempotência PASS; reconciliação de Fornecedor PASS (0 divergências, números batendo exatamente com o baseline). 2 defeitos reais encontrados e corrigidos durante a certificação: (1) comparação de watermark ignorando o fuso do servidor Linx em `SomaLinxDatasetBulkReader` (`DATA_PARA_TRANSFERENCIA` é hora local do servidor, código comparava direto contra UTC); (2) não-determinismo na aplicação de domínio sob RAW com linhas duplicadas em Incremental append-only (projetores sem desempate por "mais recente" — corrigido nos 5 consumidores RAW→REFINED do B3).

**Testes:** bateria final do Gate B3 aprovada com **1.410 testes (1.380 unitários + 30 integração), 0 falhas**; a auditoria RAW determinística subsequente, na mesma janela de fechamento, elevou a suíte para **1.393 unitários + 30 integração (1.423 total), também 0 falhas**, sem regressão. Frontend inalterado nesta rodada (211/211, herdado do fechamento dos Blocos 1–4).

**Gaps residuais (não bloqueadores, cada um com gatilho de quando tratar):** (1) `IDatasetLoadGate`/LiveRead governado (`ToolGateway`) ainda não Multi-BU-aware — tratar antes de uma 2ª BU real executar o mesmo dataset por esse caminho; (2) `RawLinxFornecedorSnapshotExecucao` ainda sem `UnidadeNegocioId` — tratar antes do onboarding operacional de uma 2ª BU no mesmo dataset; (3) `ConfiguracaoErp` existe por BU, mas os `Soma*Readers` ainda não a consomem — dívida arquitetural de evolução Multi-ERP, sem prazo atrelado a este fechamento; (4) pipeline headless de Item Fiscal (`ProcessarItensFiscaisRawParaDominioUseCase`) ainda não chama `ItemFiscal.CriarDeErp` — itens novos vindos do Linx são hoje rejeitados com ocorrência de aviso, não criados automaticamente.

**Bloco 5B (+Compras → Linx) permanece fora do fechamento desta rodada, por decisão do Product Owner** — depende de validação com especialista Visual Linx; não bloqueia o Gate B3 nem esta conclusão.

**Onda 2 (ROADMAP) permanece "Em desenvolvimento" como um todo** — Categorias/Famílias, Vínculo item×ERP×Unidade de Negócio, Centros de Custo, Projetos e Compradores permanecem Planejados, sem evidência de conclusão nesta rodada. O que está concluído/aprovado por este registro é o escopo B3/Item Fiscal (Blocos 1 a 6) e a fundação Multi-BU/Multi-ERP — não a Onda 2 inteira do roadmap. Detalhe técnico completo, cronologia integral e evidência de cada migration/teste: `applications/mais-compras/docs/cadernos/Onda-2.md`. Nenhum commit/push foi realizado por este ajuste documental.
