...
| B2.4 | Sourcing Intelligence | Contrato Canônico de Consulta CNPJ + Taxonomia de Erros | Revisar `ConsultaCnpjResultado` conforme classificação campo a campo aprovada e tipar a taxonomia de erros do provider (`CnpjInvalido`, `NaoEncontrado`, `FonteIndisponivel` etc.). | B2.3. | Draft | [B2.4](work-orders/backlog/fase-b/B2.4-ContratoCanonicoConsultaCnpjETaxonomiaDeErros.md) | ADR-0023; seções D/K do relatório de arquitetura. | Não iniciada; requer aprovação explícita do Product Owner. |
