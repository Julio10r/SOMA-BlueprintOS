...
| Onda 2 — Cadastros | B1, B2, B2.1, B2.1.1, B2.1.2, B2.1.3, B2.2 (concluídas), B3 (Gate B3 aprovado) | Fornecedores já concluídos; Item Fiscal (B3 — Discovery homologado; Blocos 1–4 concluídos/homologados; Bloco 5A certificado; Gate B3 aprovado em 04/09/2026; cadastro único, sem catálogos separados de Material/Serviço) concluído e aprovado nesta rodada, junto com a fundação Multi-BU/Multi-ERP (ver `applications/mais-compras/docs/cadernos/Onda-2.md`, "ENCERRAMENTO FORMAL DA ONDA 2"). Categorias, compradores e centros de custo (sincronização ERP própria) permanecem Planejados — a Onda 2 como um todo não está encerrada, apenas o escopo B3/Multi-BU aprovado nesta rodada. |
...
| B2.3 | Sourcing Intelligence | DocumentoFiscal — Normalização e Dígito Verificador | Unificar `Cnpj.cs`/`DocumentoFiscal.cs` em um único Value Object canônico: normalização por dígitos puros, validação de dígito verificador (CNPJ/CPF) e máscara restrita à apresentação. | B2.2 concluída. | Concluída | [B2.3](work-orders/completed/B2.3-DocumentoFiscalNormalizacaoEDigitoVerificador.md) | `DocumentoFiscal` único VO canônico; `Cnpj.cs` delega a ele; dígito verificador módulo 11 (CNPJ/CPF); `ConsultaCnpjResultado.CriarFalha` corrigido para não revalidar documento já inválido (evita BUG-3). 15 testes novos + 2 reescritos; 737 unitários + 7 integração aprovados; sem migration. | Primeira Work Order da frente Fornecedor/CNPJ (PO-3); habilita B2.4. Risco registrado: sincronização ERP de fornecedores Linx com `CGC_CPF` alfanumérico legado agora falha de forma controlada (não persiste) até o Adapter Linx (B2.9) tratar esse caso. |
| B2.4 | Sourcing Intelligence | Contrato Canônico de Consulta CNPJ + Taxonomia de Erros | Revisar `ConsultaCnpjResultado` conforme classificação campo a campo aprovada e tipar a taxonomia de erros do provider (`CnpjInvalido`, `NaoEncontrado`, `FonteIndisponivel` etc.). | B2.3. | Concluída | [B2.4](work-orders/completed/B2.4-ContratoCanonicoConsultaCnpjETaxonomiaDeErros.md) | `TipoErroConsultaCnpj` (8 valores) no Application layer, com `HttpStatusSugerido`/`PermiteRetry` derivados; `MensagemErro` passa a ser sempre derivada do tipo. BUG-3 eliminado: CNPJ com dígito verificador inválido é rejeitado localmente (`DocumentoFiscal.Create`) sem chamar a BrasilAPI; toda resposta HTTP/exceção do Adapter é classificada individualmente (404/429/401/403/5xx/payload nulo/`JsonException`/timeout/exceção inesperada). 744 unitários + 7 integração aprovados; sem migration. | Endpoint `/consulta-cnpj` mantém `200 OK` mesmo em falha (decisão documentada, preserva round-trip do objeto em outros dois endpoints); habilita B2.5. |
| B2.5 | Sourcing Intelligence | Situação Cadastral Canônica | Criar enum canônico interno de situação cadastral, eliminando o vazamento do valor numérico bruto do provider até o frontend (`StatusBadge`). | B2.4. | Concluída | [B2.5](work-orders/completed/B2.5-SituacaoCadastralCanonica.md) | Enum `SituacaoCadastralCnpj` ampliado para `Ativa, Baixada, Suspensa, Inapta, Nula, Desconhecida`; `SituacaoCadastral` passou a nullable (só preenchida em sucesso); causa raiz real do vazamento numérico identificada e corrigida (ausência de `JsonStringEnumConverter` no backend — afetava todos os enums do contrato HTTP, não só situação cadastral); `SituacaoCadastralBadge` novo componente dedicado ao CNPJ, sem tocar no `StatusBadge` genérico usado por ~15 telas de Administração; contrato TS sem `| string` residual. 15 testes backend + 10 frontend novos; 759 unitários + 7 integração; 126/126 frontend; sem migration. | Habilita B2.6. |
| B2.6 | Sourcing Intelligence | State Machine de Revisão no Frontend | Corrigir o fluxo `CadastroFornecedor.tsx` para `Idle → Validating → Consulting → Review → Persisting`, removendo a criação automática de Fornecedor ao consultar. | B2.4; B2.5. | Concluída | [B2.6](work-orders/completed/B2.6-StateMachineDeRevisaoNoFrontend.md) | State machine `Idle→Validating→Consulting→Review→Persisting→Success/Error` implementada em `CadastroFornecedor.tsx`; BUG-1 eliminado — `createSupplierDraft` deixou de ser chamado automaticamente em `handleConsult`, passando a exigir clique explícito em "Cadastrar fornecedor" (novo componente `NovoFornecedorPanel.tsx`); bloqueio de situação cadastral corrigido para `Baixada/Suspensa/Inapta` (antes só `Baixada`); proteção contra submissão duplicada (`disabled` + `useRef`). 1 teste backend + 4 testes frontend novos; 760 unitários + 7 integração; 130/130 frontend; sem migration. | Smoke test via Chrome DevTools MCP não executado (impedimento: exigiria backend real conectado a `SOMA_DESENV`, proibido de escrita); cobertura validada por testes automatizados com banco em memória/mocks. Habilita B2.7/B2.8. |
| B2.7 | Sourcing Intelligence | Proveniência Híbrida e Retenção de 180 Dias | Adicionar snapshot bruto versionado do retorno do Provider em `FornecedorCnpjConsultaHistorico`, com retenção de 180 dias e mecanismo de expurgo seguro e testável. | B2.4. | Concluída | [B2.7](work-orders/completed/B2.7-ProvenienciaHibridaERetencao180Dias.md) | `TipoErro`/`PayloadBrutoJson`/`PayloadBrutoDescartadoPorTamanho` adicionados a `FornecedorCnpjConsultaHistorico`; `ICnpjConsultaProviderComSnapshot` (interface opcional) + `BrasilApiSnapshotSanitizer` (remove QSA/segredos) na fronteira Provider→Adapter, sem alterar `ICnpjConsultaProvider.ConsultarAsync`; retenção com fronteira exata (179/180 preservados, 181 expira) via `PayloadBrutoExpirado`; expurgo idempotente via `ExpurgarPayloadBrutoConsultaCnpjUseCase`/`ExecuteUpdateAsync`; migration `AddProvenienciaHibridaConsultaCnpjB27`. 43 testes backend novos; 803 unitários + 7 integração; frontend não tocado (130/130 preservado). | Desvio de escopo documentado: snapshot gravado a cada consulta (como o restante do histórico), não apenas em criação/atualização real de Fornecedor — ver ADR-0023 addendo B2.7. Habilita B2.8. |
| B2.8 | Sourcing Intelligence | CNAE Principal | Persistir código e descrição do CNAE principal em `Fornecedor`; CNAEs secundários não entram no domínio. | B2.4. | Concluída | [B2.8](work-orders/completed/B2.8-CnaePrincipal.md) | `Fornecedor.CnaePrincipalCodigo`/`CnaePrincipalDescricao` (nullable, dígitos puros); `ConsultaCnpjResultado` ganhou os mesmos dois campos; `BrasilApiCnpjProvider` mapeia `cnae_fiscal`/`cnae_fiscal_descricao` e descarta `cnaes_secundarios` na fronteira (provado por teste); persistência só via `DefinirCnaePrincipal`, chamado exclusivamente pela operação explícita de cadastro/atualização; fornecedor existente comparado campo a campo (`FornecedorEnriquecimentoComparer`) sem atualização silenciosa; migration `AddCnaePrincipalFornecedorB28` aditiva/nullable. 7 testes backend + 3 frontend novos; 810 unitários + 7 integração; 133/133 frontend; QSA/secundários confirmados fora do domínio. | Habilita B2.9 (ainda bloqueada — validação com especialista Visual Linx pendente). |
| B2.9 | Sourcing Intelligence | Adapter Linx (Fornecedor/CNPJ) | Traduzir o domínio +Compras já corrigido para o contrato físico Linx (`CLIFOR`/`COD_CLIFOR`/`NOME_CLIFOR`, filas ETL/WETL). | B2.3 a B2.8 concluídas. | **Concluída — Implementação Técnica** (homologação manual transferida ao Gate Final da Onda 1) | [B2.9](work-orders/completed/B2.9-AdapterLinxFornecedorCnpj.md) | ADR-0023 (decisão PO-4); Gate Pré-B2.9 desbloqueado; Adapter `SomaGarantirFornecedorErpAdapter` implementado e validado contra SOMA_DESENV real em 12/08/2026, incluindo rodada de diagnóstico técnico controlado (atomicidade/rollback provados com falha real, bug de FK em ADD_ROLE encontrado e corrigido, concorrência CREATE/CREATE validada, órfãos de teste diagnosticados e removidos). Fechamento técnico em 17/08/2026: 823 testes unitários + 13 integração, build/tsc/testes/build frontend aprovados, sem migration pendente. | Decisão do PO (17/08/2026): homologação manual pelo Visual Linx real **postergada e incorporada ao Gate Final de Homologação Manual da Onda 1** (após implementações restantes da Onda 1, Design Review consolidado, correções e regressão final) — não é mais pendência isolada da B2.9. RBAC do endpoint (`Fornecedor.Editar`) aprovado pelo PO. Risco residual aceito formalmente: ausência de índice único físico em `CADASTRO_CLI_FOR.CGC_CPF` (proteção de concorrência é aplicacional, validada empiricamente). Ver relatório completo na Work Order. |
| B3 | Sourcing Intelligence | Cadastro e Integração de Item Fiscal | Criar consulta ERP, cadastro próprio (Item Fiscal — cadastro único, sem catálogos separados de Material/Serviço), seleção manual e relacionamentos com fornecedores. | B1; B2.1 e B2.2. | **Concluída — Gate B3 aprovado (04/09/2026)** | [B3](work-orders/backlog/fase-b/B3-historico-de-compras.md) | Discovery homologado pelo Product Owner (`ContratoFuncionalPreliminar-B3-ItemFiscal.md`, `B3-ConsolidacaoFinalDiscovery.md`). Bloco 1 (Conta Contábil) e Bloco 2 (Unidade de Medida) — cadastros de apoio, leitura Linx real — concluídos/homologados. Bloco 3 (Item Fiscal — domínio, CRUD local, RBAC) e Bloco 4 (Referências por Fornecedor) concluídos/homologados (commit `c2365f5`, 02/09/2026). Bloco 5A (sincronização Linx → +Compras: RAW/REFINED/reconciliação de Fornecedor, Item Fiscal, Cadastro de Apoio, Fornecedor Domínios e Item Fiscal Referência Fornecedor) implementado, auditado (auditoria RAW determinística) e certificado pela bateria final do Gate B3 (teste controlado real de ~101 Fornecedores em SOMA_DESENV, idempotência, reconciliação, 0 divergências) — commit `d8dac6ed82aa0cfaa2222b4c56b6288fbc241a77`, 04/09/2026. Bloco 5B (+Compras → Linx) permanece intencionalmente **NÃO iniciado** — depende de validação com especialista Visual Linx, **não bloqueia** o Gate B3 nem o fechamento funcional da Onda 2. Bloco 6 (Gate final) corresponde à própria bateria de certificação já concluída. | Detalhe completo em `applications/mais-compras/docs/cadernos/Onda-2.md` ("ENCERRAMENTO FORMAL DA ONDA 2" e "Gaps residuais da Onda 2"), `.ai/PROJECT_STATE.md` e `applications/mais-compras/docs/product/B3-PlanoDeImplementacao.md`. GAPs residuais não bloqueadores: `IDatasetLoadGate`/LiveRead governado ainda não Multi-BU-aware; `RawLinxFornecedorSnapshotExecucao` sem `UnidadeNegocioId`; `ConfiguracaoErp` ainda não consumida pelos leitores ERP reais — todos com gatilho definido para tratamento futuro, nenhum bloqueia esta conclusão. |
...
**Continuação do Gate Final da Onda 1 (12/08/2026) — ADR-0022, ver `.ai/DECISIONS.md` e `.ai/PROJECT_STATE.md`.** O "achado registrado para decisão de produto" acima (5 controllers com `unidadeNegocioId` no path) está **RESOLVIDO**: decisão formal do Product Owner define dois níveis administrativos (Administrador Sênior = escopo Produto, cross-BU explícito; Administrador de BU = escopo Negócio, restrito à própria BU), implementados por uma abstração central (`EscopoAdministrativoUnidadeNegocio`) e aplicados aos 5 controllers via `EscopoUnidadeNegocioPathFilter`. `PerfisController`/`UsuariosController` passam a aceitar `unidadeNegocioId` opcional por query, honrado somente para o Administrador Sênior. **#9 resolvido** — catálogo inicial de 5 Perfis de negócio (Administrador Sênior, Administrador de BU, Comprador, Aprovador, Requisitante) aprovado e implementado, seed idempotente e multi-BU consciente (`CatalogoInicialPerfisDeNegocioUseCase` + migration `AddCatalogoInicialPerfisDeNegocioO1Gate`). Testes: backend 707 unitários + 7 integração (0 falhas), frontend 116/116, builds limpos, `dotnet ef migrations has-pending-model-changes` sem pendências. **Resultado: 40/41 (97,5610% exato, exibido 98%). GATE PERMANECE ABERTO** — resta somente **#41** (validação funcional com os envolvidos; mesmo impedimento externo de VPN/banco corporativo). **Design Review NÃO foi executado. Onda 2 NÃO foi iniciada.**

---

**Validação Funcional do #41 (12/08/2026, mesma data) — VPN disponibilizada pelo PO, GATE PERMANECE ABERTO EM 40/41.** Conectividade real confirmada com `MaisComprasConnection`/`SOMA_DESENV`; ambiente real (backend+frontend+navegador) validado via Chrome DevTools MCP. 4 bugs reais encontrados e corrigidos (nenhum detectável sem conexão real): Administrador Sênior com permissões incompletas, proxy Vite sem `/me`, DEB-13 reaberta (mapeamento de coluna do Fornecedor — a conclusão anterior "não reproduz" estava errada), DEB-19 nova (tabela de Centro de Custo mal configurada). Escopo administrativo (ADR-0022) revalidado com atores/BUs reais — todos os cenários corretos. Testes: backend 713 + 7 integração (0 falhas, integração agora exercitando conexão real), frontend 116/116. **#41 não foi marcado Concluído**: sua definição formal exige sessão de validação **com os envolvidos** (stakeholders humanos) e aceite formal do Product Owner — validação técnica aprovada, aceite humano permanece como próximo passo, não autoassinado pelo agente. Detalhe completo em `.ai/PROJECT_STATE.md` e `docs/audits/O1.14-InventarioDividasEGaps.md`. **Design Review NÃO executado. Onda 2 NÃO iniciada.**

---

**Fechamento Formal da Onda 1 (12/08/2026, mesma data) — aceite do Product Owner recebido.** O Product Owner aceitou formalmente o Relatório Final da Validação Funcional do #41 e autorizou: #41 → Concluído; matriz → 41/41; progresso técnico → 100%; Gate Final → Concluído; Onda 1 → formalmente Concluída. Nenhum ambiente de Homologação exigido. Nenhuma alteração de código nesta rodada (fechamento documental) — números de teste (713 backend + 7 integração; 116 frontend) são os já produzidos na validação funcional anterior.

**Matriz final: 41/41 Concluídos. Progresso Técnico: 100%. MVP Global: 46,5122% → 47,0% exato (exibido 47%)**, mesma fórmula oficial (Foundation 20,0 + Onda 1 20,0 + Onda 2 7,0). Dívidas técnicas remanescentes preservadas (ver `docs/audits/O1.14-InventarioDividasEGaps.md`) — nenhuma foi quitada ou apagada por este fechamento. Handoff documentado (não iniciado) para o Design Review Consolidado Pós-Onda 1 em `.ai/CURRENT_SPRINT.md`. `DASHBOARD_STATE.md` não foi editado. **Design Review NÃO iniciado. Onda 2 NÃO iniciada.**

---

**DR-PosOnda1 — Correções Consolidadas do Design Review Pós-Onda 1 (17/08/2026) — CONCLUÍDA TECNICAMENTE.** Frente única de estabilização (não uma Work Order estratégica das fases A–H; não altera a matriz 41/41 da Onda 1) executando os 8 lotes do Design Review: navegação por contexto, header/identidade, padronização administrativa, correção do Review de Fornecedor sem dados (DR-10), inativação de Fornecedor em vez de exclusão física (DR-18), revisão PT-BR e verificações finais via Chrome. Regressão: backend 838/838, frontend 145/145, builds/migrations limpos. Work Order: `.ai/work-orders/completed/DR-PosOnda1-CorrecoesConsolidadas.md`. **Onda 2 e B3 permanecem NÃO iniciados. Gate Manual Final (CRUD/E2E manual + Visual Linx) permanece pendente do Product Owner.**

---

**Gate Fornecedores — APROVADO PELO PRODUCT OWNER (01/09/2026).** Correção/endurecimento do cadastro de Fornecedores (reteste pós-DR-PosOnda1) aprovado explicitamente pelo Product Owner com dispensa de novo smoke manual isolado — validação técnica (build limpo, 1147/1147 unitários, integração real, RBAC, duplicidade, regra assimétrica de inativação, +Compras↔Linx comprovados nas duas direções) considerada suficiente. Escopo específico deste cadastro — **não** equivale ao Gate Manual Final abrangente da Onda 1 acima, que permanece pendente para as demais superfícies administrativas. Cadastro não está congelado; será reexercitado dentro do workflow real de Compras quando implementado (validação integrada futura, não reabertura automática do Gate). Publicado `applications/mais-compras/docs/architecture/CadFormFactory.md` (processo reutilizável para próximos cadastros, referenciado por `Architecture.md`/`agents/README.md`). **Onda 2 permanece ativa; B3 (Materiais/Serviços) continua o próximo item planejado, ainda não iniciado.** Detalhe completo em `.ai/PROJECT_STATE.md`.

---

**B3 — Discovery homologado e Blocos 1–4 concluídos/homologados pelo Product Owner (02/09/2026).** Discovery de Item Fiscal (Cadastro e Integração de Itens) homologado — **comprovado que não existem cadastros mestres separados de Material e Serviço no Linx**: Item Fiscal é um cadastro único (`CADASTRO_ITEM_FISCAL`), correção de escopo funcional em relação à descrição original "materiais/serviços" desta Work Order. Implementação em blocos pequenos e homologáveis individualmente (`applications/mais-compras/docs/product/B3-PlanoDeImplementacao.md`):

- **Bloco 1 — Conta Contábil** (cadastro de apoio, leitura real de `CTB_CONTA_PLANO`) — Concluído/Homologado.
- **Bloco 2 — Unidade de Medida** (cadastro de apoio, leitura real de `UNIDADES`) — Concluído/Homologado, com o ajuste funcional de excluir registros com código vazio/só espaços (achado real do Linx).
- **Bloco 3 — Item Fiscal** (domínio local, CRUD completo, Conta Contábil e Unidade obrigatórias e validadas, RBAC granular por ação) — Concluído/Homologado.
- **Bloco 4 — Referências por Fornecedor** (DE/PARA local Fornecedor × código do fornecedor × Item Fiscal, base técnica para processamento futuro de XML NF-e/NFS-e — ainda não implementado) — Concluído/Homologado.

Validação técnica de cada bloco: build limpo (backend e frontend), suíte completa de testes (1211 unitários + 25 integração no fechamento do Bloco 4, mais os testes de frontend correspondentes), validação real end-to-end contra `MAISCOMPRAS` e dados reais do Linx (Contas Contábeis, Unidades, Fornecedores), RBAC comprovado, **nenhuma escrita realizada no Linx em nenhum bloco**. Commit publicado em `origin/main`: `c2365f5`. **Bloco 5A (sincronização Linx → +Compras) é o próximo item, ainda NÃO iniciado — próximo passo é a pré-validação dos dados reais de `CADASTRO_ITEM_FISCAL`/`ITEM_FISCAL_REF_FORNECEDOR` antes de implementar.** Bloco 5B e Bloco 6 (Gate final da B3) permanecem NÃO iniciados. Onda 2 permanece ativa. Detalhe completo em `.ai/PROJECT_STATE.md` e `applications/mais-compras/docs/product/B3-PlanoDeImplementacao.md`.
