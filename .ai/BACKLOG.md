...
| B2.6 | Sourcing Intelligence | State Machine de Revisão no Frontend | Corrigir o fluxo `CadastroFornecedor.tsx` para `Idle → Validating → Consulting → Review → Persisting`, removendo a criação automática de Fornecedor ao consultar. | B2.4; B2.5. | Draft | [B2.6](work-orders/backlog/fase-b/B2.6-StateMachineDeRevisaoNoFrontend.md) | ADR-0023; seção N do relatório de arquitetura. | Pode ser paralela a B2.5 se as dependências reais permitirem. |
