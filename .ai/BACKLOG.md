...
| B2.9 | Sourcing Intelligence | Adapter Linx (Fornecedor/CNPJ) | Traduzir o domínio +Compras já corrigido para o contrato físico Linx (`CLIFOR`/`COD_CLIFOR`/`NOME_CLIFOR`, filas ETL/WETL). | B2.3 a B2.8 concluídas. | Draft — Bloqueada | [B2.9](work-orders/backlog/fase-b/B2.9-AdapterLinxFornecedorCnpj.md) | ADR-0023 (decisão PO-4). | **Qualquer escrita real no Linx permanece bloqueada até sessão de validação com especialista Visual Linx.** Planejamento arquitetural autorizado; implementação de escrita, não. |
