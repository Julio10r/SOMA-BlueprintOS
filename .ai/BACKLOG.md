...
| B2.3 | Sourcing Intelligence | DocumentoFiscal — Normalização e Dígito Verificador | Unificar `Cnpj.cs`/`DocumentoFiscal.cs` em um único Value Object canônico: normalização por dígitos puros, validação de dígito verificador (CNPJ/CPF) e máscara restrita à apresentação. | B2.2 concluída. | Approved / In Progress | [B2.3](work-orders/active/B2.3-DocumentoFiscalNormalizacaoEDigitoVerificador.md) | ADR-0023 (`.ai/DECISIONS.md`); `docs/audits/Arquitetura-Fornecedor-CNPJ-Decisao.md`, seção C. | Primeira Work Order da frente Fornecedor/CNPJ (PO-3); habilita B2.4. |
