...
O Design System oficial da AZZAS 2154 usa três famílias tipográficas, todas hospedadas no Google Fonts e carregadas via `resources/design-system/fonts.css`. Ver tokens de aplicação em `resources/design-system/colors_and_type.css` e exemplos em `resources/design-system/preview/type-display.html`, `type-mono.html`, `type-ui.html`.
...
Carregamento padrão via `@import` do Google Fonts, já configurado em `resources/design-system/fonts.css`:
...
```html
<link rel="stylesheet" href="/resources/design-system/fonts.css">
```
