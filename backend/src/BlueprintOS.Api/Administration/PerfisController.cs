...
///
/// Escopo de dados: a Unidade de Negócio vem sempre de <see cref="ICurrentIdentity"/> (claim da sessão),
/// nunca do corpo ou da query string — não existe caminho pelo qual um administrador de uma Unidade de
/// Negócio leia ou altere Perfis de outra, mesmo enviando um Id válido de outra Unidade.
///
...
public static class PerfisController
{
    ...
    private static async Task<IResult> ListarPerfis(
        ICurrentIdentity identity, IListarPerfisUseCase useCase, CancellationToken ct)
    {
    ...
    private static async Task<IResult> ObterPerfil(
        Guid id, ICurrentIdentity identity, IObterPerfilUseCase useCase, CancellationToken ct)
    {
        if (!TryResolverUnidadeNegocio(identity, out var unidadeNegocioId, out var falha)) return falha!;

        var perfil = await useCase.ExecuteAsync(id, unidadeNegocioId, ct);
    ...
    private static async Task<IResult> CriarPerfil(
        PerfilUpsertRequest? request, ICurrentIdentity identity, ICriarPerfilUseCase useCase, CancellationToken ct)
    {
        if (!TryResolverUnidadeNegocio(identity, out var unidadeNegocioId, out var falha)) return falha!;
        ...
        var resultado = await useCase.ExecuteAsync(
            ParaInput(request), unidadeNegocioId, PermissoesDoAtor(identity), ct);
    ...
    private static async Task<IResult> AtualizarPerfil(
        Guid id, PerfilUpsertRequest? request, ICurrentIdentity identity, IAtualizarPerfilUseCase useCase, CancellationToken ct)
    {
        if (!TryResolverUnidadeNegocio(identity, out var unidadeNegocioId, out var falha)) return falha!;

        var resultado = await useCase.ExecuteAsync(
            id, ParaInput(request), unidadeNegocioId, PermissoesDoAtor(identity), ct);
        return resultado.Sucesso ? Results.Ok(resultado.Valor) : Traduzir(resultado);
    }

    private static async Task<IResult> AlterarStatus(
        Guid id, PerfilStatusRequest? request, ICurrentIdentity identity, IAlterarStatusPerfilUseCase useCase, CancellationToken ct)
    {
        if (!TryResolverUnidadeNegocio(identity, out var unidadeNegocioId, out var falha)) return falha!;
    ...
    /// <summary>A ausência da claim de Unidade de Negócio (esquema exclusivo de Development) é tratada como
    /// <c>403</c>: a requisição está autenticada, mas não há escopo de negócio no qual autorizá-la.
    /// Fail-closed — nunca assumir uma Unidade de Negócio padrão.</summary>
