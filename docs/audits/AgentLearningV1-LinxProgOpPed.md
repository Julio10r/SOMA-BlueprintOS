...
>
> **Atualizacao (rodada de investigacao authoritative em producao):** a rodada que tentaria confrontar estes achados com `linx-production` (`SOMA`, `192.168.0.200`) foi **bloqueada por indisponibilidade de conectividade** (`CONNECTIVITYUNAVAILABLE` apos retry automatico, em 2 tentativas independentes - erro TCP 35, servidor nao alcancavel; a credencial esta configurada, o problema e estritamente de rede/VPN ao host de producao). `SOMA_DESENV` **nao foi usado como substituto**. Nenhum item desta secao 7 foi promovido a `CONFIRMED_IN_PRODUCTION`. Relatorio completo desta tentativa: `docs/audits/LinxProgOpPed-ProductionInvestigation.md`.
