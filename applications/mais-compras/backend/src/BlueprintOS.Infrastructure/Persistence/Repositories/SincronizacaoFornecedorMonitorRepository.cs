...
public sealed class SincronizacaoFornecedorMonitorRepository(BlueprintOSDbContext db) : ISincronizacaoFornecedorMonitorRepository
{
    ...
    public Task<bool> ExisteEmAndamentoAsync(Guid unidadeNegocioId, string businessUnit, CancellationToken ct) =>
        db.SincronizacoesFornecedores.AsNoTracking().AnyAsync(x =>
            x.UnidadeNegocioId == unidadeNegocioId && x.BusinessUnit == businessUnit && x.Status == "EmAndamento", ct);
}
