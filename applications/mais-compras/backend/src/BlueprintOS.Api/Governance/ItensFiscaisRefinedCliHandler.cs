...
/// <summary>B3 — Bloco 5A (preparação de certificação final): CLI para <see cref="ProcessarItensFiscaisRawParaDominioUseCase"/>.
///
/// Onda 2 (Multi-BU/Multi-ERP, 03/09/2026): <c>--business-unit</c> é OBRIGATÓRIO — este processo é headless
/// (sem sessão de usuário), então a Business Unit da execução precisa vir explicitamente de quem dispara o
/// comando, nunca inferida. O valor é o <c>Slug</c> real de uma <c>UnidadeNegocio</c> já cadastrada (ex.:
/// "grupo-soma") — nunca um Guid hardcoded. Ausência do argumento, slug inexistente ou Unidade de Negócio
/// inativa falham fechado (exit code 1) antes de qualquer leitura/escrita de domínio.</summary>
public static class ItensFiscaisRefinedCliHandler
{
    ...
    public static async Task<int> RunAsync(string[] args, TextWriter output, IConfiguration configuration, CancellationToken cancellationToken = default)
    {
        ...
        {
            return await WriteErrorAsync(output, "UNKNOWN_VERB", "Uso: linx-itens-fiscais run --mode dry-run|apply --business-unit <slug>");
        }
...
#pragma warning restore ASP0000
        ...
        var resultado = await new ProcessarItensFiscaisRawParaDominioUseCase(db, occurrenceRepository, scope.ServiceProvider.GetRequiredService<ILogger<ProcessarItensFiscaisRawParaDominioUseCase>>())
