...
/// <summary>
/// B3 — Bloco 5A (preparação de certificação final): CLI que executa, em sequência, a sincronização dos 3
/// catálogos Linx (<see cref="SincronizarFornecedorDominiosErpUseCase"/>) e a vinculação real de Fornecedores
/// existentes a esses catálogos (<see cref="VincularFornecedorDominiosErpUseCase"/>) — conectando
/// efetivamente a infraestrutura de FornecedorDominioErp/VincularDominios que existia sem nenhum chamador.
///
...
/// </summary>
public static class FornecedorDominioErpCliHandler
{
    ...
    public static async Task<int> RunAsync(string[] args, TextWriter output, IConfiguration configuration, CancellationToken cancellationToken = default)
    {
        ...
        {
            return await WriteErrorAsync(output, "UNKNOWN_VERB", "Uso: linx-fornecedor-dominios run --mode dry-run|apply --business-unit <slug>");
        }
...
#pragma warning restore ASP0000
        ...
        var sincronizacao = await new SincronizarFornecedorDominiosErpUseCase(db, occurrenceRepository, scope.ServiceProvider.GetRequiredService<ILogger<SincronizarFornecedorDominiosErpUseCase>>())
            .ExecutarAsync(dryRun, unidadeNegocio.Id, TimeProvider.System, cancellationToken);

        var vinculacao = await new VincularFornecedorDominiosErpUseCase(db, occurrenceRepository, scope.ServiceProvider.GetRequiredService<ILogger<VincularFornecedorDominiosErpUseCase>>())
            .ExecutarAsync(sincronizacao.ExecucaoRawId, unidadeNegocio.Id, sincronizacao.CatalogoResultante, dryRun, TimeProvider.System, cancellationToken);

        await WriteAsync(output, new
        {
            sincronizacao = new
            {
                sincronizacao.ExecucaoRawId,
                sincronizacao.DryRun,
                sincronizacao.Aplicado,
                sincronizacao.TotalRaw,
                sincronizacao.Inseridos,
                sincronizacao.Atualizados,
                sincronizacao.SemAlteracao,
                sincronizacao.Rejeitados,
                sincronizacao.OcorrenciasPersistidas,
            },
            vinculacao,
        });
        return 0;
