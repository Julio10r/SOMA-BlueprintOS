...
/// <summary>B3 — Bloco 2: prova de ENFORCEMENT REAL nas rotas verdadeiras de
/// <see cref="UnidadesMedidaController"/> (mapeadas via <c>MapUnidadesMedida</c>), mesmo padrão de
/// <c>ContasContabeisRbacTests</c>: sem sessão → 401; autenticado sem <c>UnidadeMedida.Gerenciar</c> → 403;
/// autenticado com <c>UnidadeMedida.Gerenciar</c> e Unidade de Negócio resolvida → 200.</summary>
public sealed class UnidadesMedidaRbacTests : IAsyncDisposable
{
    ...
    public async Task Should_Return_401_Without_Any_Session()
    {
        ...
        Assert.Equal(HttpStatusCode.Unauthorized, (await client.GetAsync("/api/administracao/unidades-medida")).StatusCode);
    }

    [Fact]
    public async Task Should_Return_403_When_Authenticated_Without_UnidadeMedidaGerenciar()
    {
        var client = await StartAsync();
        ComSessao(client);

        Assert.Equal(HttpStatusCode.Forbidden, (await client.GetAsync("/api/administracao/unidades-medida")).StatusCode);
    }
    ...
    [Fact]
    public async Task Should_Return_200_When_Authenticated_With_UnidadeMedidaGerenciar()
    {
        var client = await StartAsync(PermissaoCatalogo.UnidadeMedidaGerenciar);
        ComSessao(client);

        Assert.Equal(HttpStatusCode.OK, (await client.GetAsync("/api/administracao/unidades-medida")).StatusCode);
    }
    ...
    private async Task<HttpClient> StartAsync(params string[] permissoesEfetivas)
    {
        ...
        builder.Services.AddScoped<IListarUnidadesMedidaUseCase, FakeListarUnidadesMedidaUseCase>();
        builder.Services.AddScoped<IAtualizarMetadadoUnidadeMedidaUseCase, FakeAtualizarMetadadoUnidadeMedidaUseCase>();
    ...
    private sealed class FakeListarUnidadesMedidaUseCase : IListarUnidadesMedidaUseCase
    {
        public Task<IReadOnlyList<UnidadeMedidaDto>> ExecuteAsync(Guid unidadeNegocioId, CancellationToken ct) =>
            Task.FromResult<IReadOnlyList<UnidadeMedidaDto>>([]);
    }

    private sealed class FakeAtualizarMetadadoUnidadeMedidaUseCase : IAtualizarMetadadoUnidadeMedidaUseCase
    {
        public Task<ErpMetadadoResultado<UnidadeMedidaDto>> ExecuteAsync(string codigoErp, UnidadeMedidaMetadadoInput input, Guid unidadeNegocioId, CancellationToken ct) =>
            Task.FromResult(ErpMetadadoResultado<UnidadeMedidaDto>.Erro(ErpMetadadoFalha.CodigoErpNaoEncontrado, "não usado neste teste"));
    }
}
